<?php
class Dangdang extends BaseAction{

	//淘宝收集数据并返回到页面
	public function item($dangdang_url){
	...
	//淘宝批量收集数据并添加到数据库
	public function betchitem($dangdang_url,$cate_id){
	...
	public function match_title($content){
		preg_match('/<div class=\"h1_title\">(.*?)<h1>(.*?)</si',$content,$result);
	...
	public function match_image($content){
		preg_match('/target=\"_blank\"><img src=\"(.*?)\" alt=\"\" id=\"largePic\"/si',$content,$result);
		isset($result) ? $image = $result[1] : $image = '';
		return $image;
	}
	
	//正则获取商品价格(抢购)
	public function match_lprice($content){
		preg_match('/<span class=\"promotions_price_d\">(.*?)<b>(.*?)<\/b>/si',$content,$result);
	...
	public function match_price($content){
		preg_match('/<p class=\"price_d\">(.*?)>(.*?)<\/span>/si',$content,$result);
		isset($result) ? $price = ltrim(iconv('GBK', 'UTF-8', $result[2]),'￥') : $price = '';
		return $price;
	}
	
	//正则获取商品ID
	public function match_item_id($url){
		preg_match('/product_id=(\d+)/si',$url,$result);
		isset($result) ? $item_id = $result[1] : $item_id = '';
		return $item_id;
	}
}
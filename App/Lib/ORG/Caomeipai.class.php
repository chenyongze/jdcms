<?php
class Caomeipai extends BaseAction{

	//淘宝收集数据并返回到页面
	public function item($caomeipai_url){
	...
	//淘宝批量收集数据并添加到数据库
	public function betchitem($caomeipai_url,$cate_id){
	...
	public function match_title($content){
		preg_match('/<h1 class=\"product-name\">(.*?)<span class=\"size\">(.*?)<\/span>/si',$content,$result);
	...
	public function match_image($content){
		preg_match('/<div class=\"thumb\">(.*?)src=\"(.*?)\"\/>/si',$content,$result);
	...
	public function match_price($content){
		preg_match('/<i class=\"cmp-price\">(.*?)<strong>(.*?)<\/strong>/si',$content,$result);
	...
	public function match_item_id($url){
		preg_match('/goods(\d+)/si',$url,$result);

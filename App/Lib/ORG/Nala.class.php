<?php
class Nala extends BaseAction{

	//淘宝收集数据并返回到页面
	public function item($nala_url){
	...
	//淘宝批量收集数据并添加到数据库
	public function betchitem($nala_url,$cate_id){
	...
	public function match_title($content){
		preg_match('/<div class=\"product_title\">(.*?)<h2 class=\"bold\">(.*?)<\/h2>/si',$content,$result);
	...
	public function match_image($content){
		preg_match('/rel=\"productimg\"> <img(.*?)src=\"(.*?)\"/si',$content,$result);
	...
	public function match_price($content){
		preg_match('/<span class=\"nalaprice\"><b>(.*?)<\/b>(.*?)<\/span>/si',$content,$result);
		isset($result) ? $price = $result[1].$result[2] : $price = '';
		return $price;
	}

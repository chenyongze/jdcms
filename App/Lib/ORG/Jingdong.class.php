<?php
class Jingdong extends BaseAction{

	//淘宝收集数据并返回到页面
	public function item($jingdong_url){
		...
		//获得移动端URL
		$url = 'http://m.360buy.com/product/'.$item_id.'.html';
		...
		//获得商品价格
		$file = fopen($jingdong_url,'r');
		$item = fread($file,'5120');
	...
	//淘宝批量收集数据并添加到数据库
	public function betchitem($jingdong_url,$cate_id){
	...
	public function match_item_id($url){
		preg_match('/product\/(\d+)/si',$url,$result);
	...
	public function match_title($content){
		preg_match('/<title>(.*?) - 京东商城手机版<\/title>/si',$content,$result);
	...
	public function match_image($content){
		preg_match('/<div class=\'p-img\'>(.*?)<img src=\'(.*?)\' \/>/si',$content,$result);
	...
	public function match_price($content){
		preg_match('/price:\'(.*?)\'/si',$content,$result);

<?php
class Mbaobao extends BaseAction{

	//淘宝收集数据并返回到页面
	public function item($mbaobao_url){
	...
	//淘宝批量收集数据并添加到数据库
	public function betchitem($mbaobao_url,$cate_id){
	...
	public function match_title($content){
		preg_match('/<h1 class=\"goods-name\">(.*?)<span class=\"goods-name-sub\">(.*?)<\/span>/si',$content,$result);
		isset($result) ? $title = trim(addslashes($result[1])).' '.trim(addslashes($result[2])) : $title = '';
		return $title;
	}
	
	//正则获取商品图片
	public function match_image($content){
		preg_match('/458\"><img src=\"(.*)\" width=\"480\" height=\"480\" alt=\"\" class=\"js_goods_spic_url\">/si',$content,$result);
	...
	public function match_price($content){
		preg_match('/<span class=\"price-num\">(.*?)<\/span>/si',$content,$result);
	...
	public function match_item_id($content){
		preg_match('/<div class=\"goods-number\">(.*?)(\d+)<\/div>/si',$content,$result);
